...
export class StatueCharacter extends Character implements Examine, Pickup{
    ...
    public examine(): ActionResult | undefined {
        return new TextActionResult(["Its an old <blue>Stone Statue</blue>..."]);
    }
    ...
    public talk(choiceId?: number | undefined): ActionResult | undefined {
        ...
        if (choiceId === 4){
            return new TextActionResult(["I am as old as this <blue>Aztec Tempel</blue> itself"]);
        }
        if (choiceId === 5){
            return new TextActionResult(["I wish i could leave but i am a <blue>Satue</blue> i cant move"]);
        }
        if (choiceId === 6){
            playerSession.aztecTalkValue ++;
            return new TextActionResult(["That is my <blue>Little Brother</blue> you can talk to him if you want but he is really annoying"]);
        }
        if (choiceId === 9){
        ...
        if (choiceId === 12){
            return new TextActionResult(["Its behind that big rock but you will need a <blue>Tool</blue> to break it"]);
            
        }
        if(choiceId === 8){
            if (playerSession.aztecTalkValue > 1) {
                options.push(new TalkChoiceAction(9, "Did you talk to my <blue>Little Brother</blue>?"));
            }
