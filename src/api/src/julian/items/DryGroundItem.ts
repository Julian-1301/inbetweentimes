...
export class DrygroundItem extends Item implements Examine, Pickup {
    ...
    public examine(): ActionResult | undefined {
        const playerSession: PlayerSession = getPlayerSession();
        if (playerSession.drygroundValue === 0) {
        return new TextActionResult(["This <blue>Dry Ground</blue> looks like it could use some life,", "It currently looks too harsh for anything to survive though."]);
        } else if (playerSession.drygroundValue === 1) {
            return new TextActionResult(["The <blue>Plant</blue> looks really thirsty.", "Maybe you can find a way to give it some life?"]);
        } else {
